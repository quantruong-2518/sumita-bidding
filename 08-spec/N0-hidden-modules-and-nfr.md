...
> ⚠️ **SCOPE-GUARD (đọc trước khi đưa vào SOW/báo giá):** các hạng mục cột **FB** — đặc biệt **N7 Security (toàn bộ)**, **N8.5–N8.7** (uptime · error-monitoring · RUM) và **N10 Reliability/Ops (toàn bộ)** — chỉ phát sinh khi **full-build** (DECISIONS #1). Với giả định mặc định **thiết kế-only**, **KHÔNG** tính các mục này vào SOW/quote; chỉ nêu như khuyến nghị vận hành để khách biết. Phần Design-only cần lưu ý chỉ là các mục cột **DO** (UX/a11y/perf-thiết-kế/SEO-on-page/event taxonomy ở mức đặc tả).
